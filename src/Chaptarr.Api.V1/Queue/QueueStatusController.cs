...
namespace Chaptarr.Api.V1.Queue
{
                               ...
                               IHandle<QueueUpdatedEvent>, IHandle<PendingReleasesUpdatedEvent>
    {
        ...
        [HttpGet]
        public QueueStatusResource GetQueueStatus([FromQuery] string mediaType = null)
        {
            _broadcastDebounce.Pause();

            var queue = QueueMediaTypeFilter.FilterByMediaType(_queueService.GetQueue(), mediaType).ToList();
            var pending = QueueMediaTypeFilter.FilterByMediaType(_pendingReleaseService.GetPendingQueue(), mediaType).ToList();
