...
namespace Chaptarr.Http.ClientSchema
{
    ...
    {
        private const string SensitiveValuePlaceholder = "********";
        ...
        public static List<Field> ToSchema(object model)
        {
            ...
            foreach (var mapping in mappings)
            {
                ...
                var value = mapping.GetterFunc(model);

                field.Value = mapping.IsSensitive && value != null && !value.Equals(string.Empty)
        ...
        public static object ReadFromSchema(List<Field> fields, Type targetType)
        {
            ...
            return ReadFromSchema(fields, target, mappings);
        }
        ...
        public static object ReadFromSchema(List<Field> fields, object target)
        {
            ...
            var mappings = GetFieldMappings(target.GetType());
            return ReadFromSchema(fields, target, mappings);
        }
            ...
            foreach (var property in GetProperties(type))
            {
                ...
                if (propertyInfo.PropertyType.IsSimpleType())
                {
                    ...
                    var field = new Field
                    {
                        ...
                        Privacy = fieldAttribute.Privacy != PrivacyLevel.Normal
                            ? fieldAttribute.Privacy.ToString().FirstCharToLower()
                            : null,
                    ...
                    var isSensitive = fieldAttribute.Privacy == PrivacyLevel.Password ||
                                      fieldAttribute.Privacy == PrivacyLevel.ApiKey ||
                                      fieldAttribute.Type == FieldType.Password;
        ...
        private static object ReadFromSchema(List<Field> fields, object target, FieldMapping[] mappings)
        {
            foreach (var mapping in mappings)
            {
                ...
                if (mapping.IsSensitive && IsSensitivePlaceholder(field.Value))
                {
                    continue;
                }
        ...
        private static bool IsSensitivePlaceholder(object fieldValue)
        {
            if (fieldValue == null)
            {
                return false;
            ...
            {
                return strValue == SensitiveValuePlaceholder;
            }

            if (fieldValue is JsonElement element && element.ValueKind == JsonValueKind.String)
            {
                return element.GetString() == SensitiveValuePlaceholder;
            }
