...
namespace NzbDrone.Mono.Disk
{
    public class DiskProvider : DiskProviderBase
    {
        ...
        public override int? GetFileLinkCount(string path)
        {
            ...
            return checked((int)fileStat.st_nlink);
        }
                ...
                if (exists && dstInfo.Length == srcInfo.Length)
                {
                    ...
                    if (checkLength > 0)
                    {
                        ...
                        using (var dstStream = new FileStream(destination, FileMode.Open, FileAccess.Read))
                        {
                            ...
                            // Read Exactly is used here to solve CA2022 for reading into a buffer of fixed length
                            // Without then checking the length of what was read
