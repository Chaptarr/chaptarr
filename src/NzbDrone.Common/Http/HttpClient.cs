...
namespace NzbDrone.Common.Http
{
    ...
    public class HttpClient : IHttpClient
    {
            ...
            Logger logger)
        {
            ...
            // This does nothing now, there's a way to mess with the default connection limit somewhere,
        ...
        {
            EnsureHttpOrHttps(request.Url);
            if (IsCloudMetadataTarget(request.Url))
            {
                throw new WebException($"Request target '{request.Url}' is not allowed.", WebExceptionStatus.ProtocolError);
            }
            ...
            if (request.AllowAutoRedirect && response.HasHttpRedirect)
            {
                ...
                do
                {
                    ...
                    {
                        throw new WebException($"Redirect response from {request.Url} is missing a Location header.", WebExceptionStatus.ProtocolError);
                    }

                    var nextUrl = request.Url + new HttpUri(location);

                    EnsureSafeRedirectTarget(nextUrl);
        ...
        private static void EnsureHttpOrHttps(HttpUri url)
        {
            if (url == null || url.Scheme.IsNullOrWhiteSpace())
            {
                throw new WebException("Request URL must be absolute and include a scheme.", WebExceptionStatus.ProtocolError);
            }

            if (!IsHttpOrHttps(url.Scheme))
            {
                throw new WebException($"Unsupported URL scheme '{url.Scheme}'. Only http and https are supported.", WebExceptionStatus.ProtocolError);
            }
        }
        ...
        private static void EnsureSafeRedirectTarget(HttpUri url)
        {
            EnsureHttpOrHttps(url);

            if (IsCloudMetadataTarget(url))
            {
                throw new WebException($"Redirect target '{url}' is not allowed.", WebExceptionStatus.ProtocolError);
            }
        }

        private static bool IsCloudMetadataTarget(HttpUri url)
        {
            return CloudMetadataTargetPolicy.IsBlocked(url?.Host);
        }
        ...
        private CookieContainer InitializeRequestCookies(HttpRequest request)
        {
            lock (_cookieContainerCache)
            {
                ...
                var presistentContainer = _cookieContainerCache.Get(GetCookieContainerKey(request), () => new CookieContainer());
        ...
        private void HandleResponseCookies(HttpResponse response, CookieContainer container)
        {
            ...
            if (response.Request.StoreResponseCookie)
            {
                lock (_cookieContainerCache)
                {
                    var persistentCookieContainer = _cookieContainerCache.Get(GetCookieContainerKey(response.Request), () => new CookieContainer());
        ...
        private string GetCookieContainerKey(HttpRequest request)
        {
            var clientScope = GetType().FullName ?? nameof(HttpClient);
            return request?.RateLimitKey.IsNotNullOrWhiteSpace() == true
                ? $"container:{clientScope}:{request.RateLimitKey}"
                : $"container:{clientScope}";
        }
