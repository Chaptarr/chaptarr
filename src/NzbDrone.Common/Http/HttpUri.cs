...
namespace NzbDrone.Common.Http
{
    ...
    {
        private static readonly Regex RegexUri = new Regex(@"^(?:(?<scheme>[a-z]+):)?(?://(?<host>[-_A-Z0-9.]+|\[[[A-F0-9:]+\])(?::(?<port>[0-9]{1,6}))?)?(?<path>(?:(?:(?<=^)|/+)[^/?#\r\n]+)+/*|/+)?(?:\?(?<query>[^#\r\n]*))?(?:\#(?<fragment>.*))?$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private void Parse()
        {
            ...
            if (port.Success)
            {
                var portValue = int.Parse(port.Value);
                if (portValue < 1 || portValue > 65535)
                {
                    throw new ArgumentException($"Invalid port number {portValue}. Port must be between 1 and 65535. Uri: {_uri}");
                }
            }
