...
namespace NzbDrone.Common.Instrumentation
{
    public static class NzbDroneLogger
    {
        ...
        private static void RegisterConsole()
        {
            ...
            coloredConsoleTarget.Layout = new CleanseLayout
            {
                Inner = "[${level}] ${logger}: ${message} ${onexception:inner=${newline}${newline}[v${assembly-version}] ${exception:format=ToString}${newline}${exception:format=Data}${newline}}"
            };
        ...
        {
            RegisterAppFile(appFolderInfo, "appFileInfo", "chaptarr.txt", 5, LogLevel.Info);
            RegisterAppFile(appFolderInfo, "appFileDebug", "chaptarr.debug.txt", 50, LogLevel.Off);
            RegisterAppFile(appFolderInfo, "appFileTrace", "chaptarr.trace.txt", 50, LogLevel.Off);
            RegisterMatchingFile(appFolderInfo);
        }
        ...
        private static void RegisterMatchingFile(IAppFolderInfo appFolderInfo)
        {
            var fileTarget = new NzbDroneFileTarget();

            fileTarget.Name = "appFileMatching";
            ...
            var matchingLoggers = new[]
            {
                "NzbDrone.Core.MediaFiles.BookImport.*",
                "NzbDrone.Core.MediaFiles.AudioTagService",
                "NzbDrone.Core.MediaFiles.EbookTagService",
                "NzbDrone.Core.Books.Services.RefreshBookService",
                "NzbDrone.Core.Books.Services.RefreshAuthorService",
                "MatchingLogger"
            };

            foreach (var loggerPattern in matchingLoggers)
            {
                var loggingRule = new LoggingRule(loggerPattern, LogLevel.Debug, fileTarget);
                LogManager.Configuration.LoggingRules.Add(loggingRule);
            }

            LogManager.Configuration.AddTarget("appFileMatching", fileTarget);
        }
