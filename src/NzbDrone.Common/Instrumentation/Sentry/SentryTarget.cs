...
namespace NzbDrone.Common.Instrumentation.Sentry
{
    ...
    public class SentryTarget : TargetWithLayout
    {
        ...
        // Microsoft.Data.Sqlite exposes integer error codes via SqliteException.SqliteErrorCode
        // Define primary result codes we want to filter out as noisy/non-actionable.
        ...
        private const int SQLITE_AUTH = 23;

        private static readonly HashSet<int> FilteredSqliteErrors = new HashSet<int>
        {
            SQLITE_BUSY,
            SQLITE_LOCKED,
            SQLITE_PERM,
        ...
        public bool IsSentryMessage(LogEventInfo logEvent)
        {
            ...
            if (logEvent.Level >= LogLevel.Error && logEvent.Exception != null)
            {
                if (FilterEvents)
                {
                    ...
                    if (sqlEx != null)
                    {
                        // Use primary code for filtering (extended codes pack primary in low 8 bits)
                        var primary = sqlEx.SqliteErrorCode & 0xFF;
                        if (FilteredSqliteErrors.Contains(primary))
                        {
                            return false;
                        }
                    }
        ...
        protected override void Write(LogEventInfo logEvent)
        {
            ...
            try
            {
                ...
                foreach (var property in logEvent.Properties)
                {
                    ...
                    if (key.IsNullOrWhiteSpace() || key.Equals("Sentry", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }

                    extras[key] = property.Value?.ToString();
                }
                ...
                if (logEvent.Exception != null)
                {
                    foreach (DictionaryEntry data in logEvent.Exception.Data)
                    {
                        ...
                        extras[key] = data.Value?.ToString();
                    }
                }
