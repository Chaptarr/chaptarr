...
namespace NzbDrone.Core.MediaFiles
{
        ...
        private List<ImportResult> ProcessPath(DownloadedBooksScanCommand message)
        {
            ...
            if (message.DownloadClientId.IsNotNullOrWhiteSpace())
            {
                var trackedDownload = _trackedDownloadService.Find(message.DownloadClientId);
	                ...
	                if (trackedDownload != null)
	                {
	                    ...
	                    var importMode = _downloadImportModeResolver.Resolve(message.ImportMode, trackedDownload.DownloadItem);
	                    var importResults = _downloadedTracksImportService.ProcessPath(message.Path, importMode, trackedDownload.RemoteBook?.Author, trackedDownload.DownloadItem, trackedDownload.RemoteBook);
            ...
            return _downloadedTracksImportService.ProcessPath(
                message.Path,
                message.ImportMode,
                requireDefaultRootFolderForMissingAuthors: message.RequireDefaultRootFolderForMissingAuthors);
        }

        public void Execute(DownloadedBooksScanCommand message)
        {
            _logger.Debug("[IMPORT-TRIGGER] DownloadedBooksCommandService executing DownloadedBooksScanCommand - path: {0}, downloadClientId: {1}, importMode: {2}", message.Path ?? "null", message.DownloadClientId ?? "null", message.ImportMode);
