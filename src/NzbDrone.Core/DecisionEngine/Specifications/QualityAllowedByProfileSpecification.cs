    ...
    public class QualityAllowedByProfileSpecification : IDecisionEngineSpecification
    {
        ...
        public virtual Decision IsSatisfiedBy(RemoteBook subject, SearchCriteriaBase searchCriteria)
        {
            if (_logger.IsTraceEnabled)
            {
                ...
                _logger.Trace("[QUALITY_PROFILE_CHECK] Author profile IDs:");
                _logger.Trace("  - AudiobookQualityProfileId: {0}", subject.Author?.AudiobookQualityProfileId ?? -999);
                _logger.Trace("  - EbookQualityProfileId: {0}", subject.Author?.EbookQualityProfileId ?? -999);

                _logger.Trace("[QUALITY_PROFILE_CHECK] Author profile objects:");
                _logger.Trace("  - AudiobookQualityProfile: {0}",
                    subject.Author?.AudiobookQualityProfileId.HasValue == true ?
                        ...
                        "NULL");
                _logger.Trace("  - EbookQualityProfile: {0}",
            ...
            _logger.Trace("[QUALITY_PROFILE_CHECK] Calling GetQualityProfileForQuality for quality ID {0}...",
                subject.ParsedBookInfo.Quality.Quality.Id);
            var profile = subject.Author.GetQualityProfileForQuality(subject.ParsedBookInfo.Quality.Quality);

            if (profile == null)
            {
                _logger.Trace("[QUALITY_PROFILE_CHECK] GetQualityProfileForQuality returned NULL!");
                _logger.Trace("[QUALITY_PROFILE_CHECK] This means no profile is configured for quality '{0}' (ID: {1})",
                    subject.ParsedBookInfo.Quality.Quality.Name,
                    subject.ParsedBookInfo.Quality.Quality.Id);
                _logger.Trace("[QUALITY_PROFILE_CHECK] Creating SOFT FILTER rejection");
            ...
            _logger.Trace("[QUALITY_PROFILE_CHECK] Found profile: '{0}' (ID: {1})", profile.Name, profile.Id);
            _logger.Trace("[QUALITY_PROFILE_CHECK] Profile Items Count: {0}", profile.Items?.Count ?? 0);

            if (_logger.IsTraceEnabled && profile.Items != null)
            {
                var allowedQualities = profile.Items
                    .Where(i => i.Allowed)
                    .SelectMany(i => i.GetQualities())
                    .ToList();
                _logger.Trace("[QUALITY_PROFILE_CHECK] Allowed qualities in profile: [{0}]",
            ...
            {
                _logger.Trace("[QUALITY_PROFILE_CHECK] Quality {0} REJECTED - not allowed in profile", subject.ParsedBookInfo.Quality);
                _logger.Trace("[QUALITY_PROFILE_CHECK] QualityOrGroup at index {0}: Allowed={1}, Name={2}", qualityIndex.Index, qualityOrGroup.Allowed, qualityOrGroup.Name ?? qualityOrGroup.Quality?.Name ?? "Unknown");
            ...
            _logger.Trace("[QUALITY_PROFILE_CHECK] Quality {0} ACCEPTED - allowed in profile", subject.ParsedBookInfo.Quality);
