    ...
    public class Aria2 : TorrentClientBase<Aria2Settings>
    {
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            foreach (var torrent in torrents)
            {
                ...
                var outputPath = RemapRemoteToLocal(new OsPath(GetOutputPath(torrent)));
        ...
        public override DownloadClientItem GetImportItem(DownloadClientItem item, DownloadClientItem previousImportAttempt)
        {
            ...
            {
                _logger.Debug("No torrent hash found for torrent {0} in Aria2; using existing output path.", item.Title);
                return WithoutFileList(result);
            }

            List<Aria2Status> torrents;
            try
            {
            ...
            {
                _logger.Debug(ex, "Unable to fetch Aria2 torrent status for torrent {0}; using existing output path.", item.Title);
                return WithoutFileList(result);
            }

            var torrent = torrents.FirstOrDefault(t => string.Equals(t.InfoHash, item.DownloadId, StringComparison.InvariantCultureIgnoreCase));

            if (torrent == null || torrent.Gid.IsNullOrWhiteSpace())
            {
                _logger.Debug("No torrent status found for torrent {0} in Aria2; using existing output path.", item.Title);
                return WithoutFileList(result);
            }
            ...
            try
            {
                files = _proxy.GetFiles(Settings, torrent.Gid);
            }
            catch (Exception ex)
            {
                _logger.Debug(ex, "Unable to fetch Aria2 file list for torrent {0}; using existing output path.", item.Title);
                return WithoutFileList(result);
            }

            if (files == null || files.Length == 0)
            {
                _logger.Debug("No files found for torrent {0} in Aria2", item.Title);
                return WithoutFileList(result);
            }

            if (files.FirstOrDefault()?.Path?.Contains("[METADATA]") == true)
            {
                _logger.Debug("Torrent {0} in Aria2 is still resolving metadata; using existing output path.", item.Title);
                return WithoutFileList(result);
            }
            ...
            var filePaths = files
                ...
                .Select(p => RemapRemoteToLocal(new OsPath(p)).FullPath)
                .Where(p => p.IsNotNullOrWhiteSpace())
            ...
            if (!filePaths.Any())
            {
                _logger.Debug("No selected file paths found for torrent {0} in Aria2", item.Title);
                return WithoutFileList(result);
            }
        ...
        public override DownloadClientInfo GetStatus()
        {
            ...
            return new DownloadClientInfo
            {
                ...
                OutputRootFolders = new List<OsPath> { RemapRemoteToLocal(new OsPath(destDir["dir"])) }
            };
        }
        ...
        private static DownloadClientItem WithoutFileList(DownloadClientItem item)
        {
            item.FilePaths = null;
