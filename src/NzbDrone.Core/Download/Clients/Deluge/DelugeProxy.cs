...
namespace NzbDrone.Core.Download.Clients.Deluge
{
    public interface IDelugeProxy
    {
        ...
        DelugeTorrentDetails GetTorrentDetails(string hash, DelugeSettings settings);
    ...
    public class DelugeProxy : IDelugeProxy
    {
        ...
        {
            return ProcessRequest<DelugeTorrentDetails>(settings,
                "core.get_torrent_status",
                hash,
                new[] { "files", "file_priorities", "save_path" });
        }
        ...
        private JsonRpcResponse<TResult> ExecuteRequest<TResult>(JsonRpcRequestBuilder requestBuilder, string method, params object[] arguments)
        {
            ...
            catch (HttpRequestException ex)
            {
                ...
                {
                    throw new DownloadClientUnavailableException($"Connection refused to Deluge. Please verify the host and port are correct.", ex);
                }
                else if (ex.Message.Contains("No such host"))
                {
                    throw new DownloadClientUnavailableException("Unable to resolve hostname. Please verify the Deluge hostname is correct.", ex);
                }

                throw new DownloadClientUnavailableException($"Unable to connect to Deluge: {ex.Message}", ex);
            }
        }
