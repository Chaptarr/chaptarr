...
namespace NzbDrone.Core.Download.Clients.DownloadStation.Proxies
{
    public class DownloadStationTaskProxyV1 : DiskStationProxyBase, IDownloadStationTaskProxy
    {
        ...
        {
            var requestBuilder = BuildRequest(settings, "getinfo", 1);
            requestBuilder.AddQueryParam("id", downloadId);
            requestBuilder.AddQueryParam("additional", "file");

            var response = ProcessRequest<DownloadStationTaskInfoResponse>(requestBuilder, $"get files for task {downloadId}", settings);

            return response.Data.Tasks.FirstOrDefault()?.Additional?.Files ?? new List<DownloadStationTaskFile>();
        }
