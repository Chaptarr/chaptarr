...
namespace NzbDrone.Core.Download.Clients.DownloadStation
{
    public class UsenetDownloadStation : UsenetClientBase<DownloadStationSettings>
    {
        ...
        public override string Name => "Download Station (Usenet)";
        ...
        {
            _logger.Debug("Download Station NZB task file lists are not final extracted outputs for {0}; using completed output disk snapshot.", item.Title);
            return item.Clone();
        }
        ...
        protected ValidationFailure TestOutputPath()
        {
            try
            {
                ...
                if (downloadDir != null)
                {
                    ...
                    var fieldName = Settings.TvDirectory.IsNotNullOrWhiteSpace() ? nameof(Settings.TvDirectory) : nameof(Settings.AudiobookCategory);
