...
namespace NzbDrone.Core.Download.Clients.QBittorrent
{
    ...
    public class QBittorrentProxyV1 : IQBittorrentProxy
    {
        ...
        public List<QBittorrentTorrent> GetTorrents(QBittorrentSettings settings, string category = null)
        {
            ...
            var effectiveCategory = category ?? settings.MusicCategory;
            if (effectiveCategory.IsNotNullOrWhiteSpace())
            {
                request.AddQueryParam("label", effectiveCategory);
                request.AddQueryParam("category", effectiveCategory);
            }
        ...
        public void AddTorrentFromUrl(string torrentUrl, TorrentSeedConfiguration seedConfiguration, QBittorrentSettings settings, string category = null)
        {
            ...
            if (effectiveCategory.IsNotNullOrWhiteSpace())
            {
                request.AddFormParameter("category", effectiveCategory);
            }
