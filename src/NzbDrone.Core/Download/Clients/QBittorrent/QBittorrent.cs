...
{
    public class QBittorrent : TorrentClientBase<QBittorrentSettings>, IPreserveDownloadClientItemAfterImport
    {
        ...
        public override void MarkItemAsImported(DownloadClientItem downloadClientItem)
        {
                ...
                !postImportCategory.Equals(downloadClientItem.Category, StringComparison.InvariantCultureIgnoreCase))
            {
                try
                {
                    Proxy.SetTorrentLabel(downloadClientItem.DownloadId.ToLower(), postImportCategory, Settings);
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            if (configuredCategories.Any())
            {
                foreach (var category in configuredCategories)
                {
                    torrents.AddRange(Proxy.GetTorrents(Settings, category));
                }

                torrents = torrents
                    .GroupBy(t => t.Hash, StringComparer.InvariantCultureIgnoreCase)
                    .Select(g => g.First())
                    .ToList();
            }
            else
            ...
            foreach (var torrent in torrents)
            {
                ...
                if (version >= new Version("2.6.1") && item.Status == DownloadItemStatus.Completed)
                {
                    ...
                    {
                        item.OutputPath = RemapRemoteToLocal(new OsPath(torrent.ContentPath));
                    }
                    else
                    {
                        item.Status = DownloadItemStatus.Warning;
        ...
        public override DownloadClientItem GetImportItem(DownloadClientItem item, DownloadClientItem previousImportAttempt)
        {
            ...
            {
                _logger.Debug("No torrent hash found for torrent {0} in qBittorrent; using existing output path.", item.Title);
                return result;
            }
            ...
            try
            {
                files = Proxy.GetTorrentFiles(hash, Settings);
            }
            catch (Exception ex)
            {
                _logger.Debug(ex, "Unable to fetch qBittorrent file list for torrent {0}; using existing output path.", item.Title);
                return result;
            }

            var selectedFiles = files?
                .Where(f => f != null && f.Priority > 0)
                .ToList() ?? new List<QBittorrentTorrentFile>();
            ...
            try
            {
                properties = Proxy.GetTorrentProperties(hash, Settings);
            }
            catch (Exception ex)
            {
                _logger.Debug(ex, "Unable to fetch qBittorrent save path for torrent {0}; using existing output path.", item.Title);
                return result;
            }

            if (properties?.SavePath.IsNullOrWhiteSpace() != false)
            {
                _logger.Debug("No save path found for torrent {0} in qBittorrent; using existing output path.", item.Title);
                return result;
            }
            ...
            {
                _logger.Debug("No usable file paths found for torrent {0} in qBittorrent", item.Title);
                return result;
            }
            ...
            var relativePath = new OsPath(selectedFiles[0].Name);
            ...
            result.OutputPath = RemapRemoteToLocal(outputPath);
        ...
        public override DownloadClientInfo GetStatus()
        {
            ...
            if (configuredCategories.Any() && version >= Version.Parse("2.0"))
            {
                var labels = Proxy.GetLabels(Settings);

                foreach (var category in configuredCategories)
                {
                    if (!labels.TryGetValue(category, out var label) || label.SavePath.IsNullOrWhiteSpace())
                    {
                        outputRootFolders.Add(destDir);
                        continue;
                    }
                    ...
                    {
                        outputRootFolders.Add(labelDir);
                    }
                    else
                    {
                        outputRootFolders.Add(destDir + labelDir);
                    }
                }
            }
            else
        ...
        private ValidationFailure TestConnection()
        {
            try
            {
                ...
                else if (version < Version.Parse("1.6"))
                {
                    ...
                    if (Settings.AudiobookCategory.IsNotNullOrWhiteSpace() || Settings.EbookCategory.IsNotNullOrWhiteSpace())
                    {
                ...
                else if (Settings.AudiobookCategory.IsNullOrWhiteSpace() && Settings.EbookCategory.IsNullOrWhiteSpace())
                {
                    // warn if labels are supported, but category is not provided
                    return new NzbDroneValidationFailure(nameof(QBittorrentSettings.AudiobookCategory), "Category is recommended")
                    {
                        IsWarning = true,
                        DetailedDescription = "Chaptarr will not attempt to import completed downloads without a category."
                    };
                }
                ...
                if (RemovesCompletedDownloads(config))
                {
                    ...
                    {
                        DetailedDescription = "Chaptarr will be unable to perform Completed Download Handling as configured. You can fix this in qBittorrent ('Tools -> Options...' in the menu) by changing 'Options -> BitTorrent -> Share Ratio Limiting' from 'Remove them' to 'Pause them'."
                    };
                }
            }
        ...
        {
            if (Settings.AudiobookCategory.IsNullOrWhiteSpace() &&
                Settings.EbookCategory.IsNullOrWhiteSpace() &&
                Settings.AudiobookImportedCategory.IsNullOrWhiteSpace() &&
                Settings.EbookImportedCategory.IsNullOrWhiteSpace())
            {
                return null;
            }
            ...
            foreach (var pair in new[]
                     {
                         new { Field = nameof(QBittorrentSettings.AudiobookCategory), Value = Settings.AudiobookCategory },
                         new { Field = nameof(QBittorrentSettings.EbookCategory), Value = Settings.EbookCategory },
                         new { Field = nameof(QBittorrentSettings.AudiobookImportedCategory), Value = Settings.AudiobookImportedCategory },
                         new { Field = nameof(QBittorrentSettings.EbookImportedCategory), Value = Settings.EbookImportedCategory }
                     })
            {
                if (pair.Value.IsNullOrWhiteSpace() || labels.ContainsKey(pair.Value))
                {
                    continue;
                }

                Proxy.AddLabel(pair.Value, Settings);
                labels = Proxy.GetLabels(Settings);

                if (!labels.ContainsKey(pair.Value))
                {
                    return new NzbDroneValidationFailure(pair.Field, "Configuration of label failed")
                    {
                        DetailedDescription = "Chaptarr was unable to add the label to qBittorrent."
                    };
                }
            }
