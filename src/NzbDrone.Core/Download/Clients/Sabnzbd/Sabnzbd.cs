    ...
    public class Sabnzbd : UsenetClientBase<SabnzbdSettings>
    {
        ...
        private IEnumerable<DownloadClientItem> GetHistory()
        {
            ...
            foreach (var sabHistoryItem in sabHistory.Items)
            {
                ...
                var outputPath = RemapRemoteToLocal(new OsPath(sabHistoryItem.Storage));
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            {
                if (IsConfiguredCategory(downloadClientItem.Category))
                {
        ...
        private bool IsConfiguredCategory(string itemCategory)
        {
            var configuredCategories = new[] { Settings.AudiobookCategory, Settings.EbookCategory };

            if (itemCategory == "*")
            {
                // SAB uses "*" for the default category when none is specified.
                return configuredCategories.Any(c => c.IsNullOrWhiteSpace());
            }

            if (itemCategory.IsNullOrWhiteSpace())
            {
                return configuredCategories.Any(c => c.IsNullOrWhiteSpace());
            }

            return configuredCategories.Any(c => c.IsNotNullOrWhiteSpace() && c.Equals(itemCategory, StringComparison.InvariantCultureIgnoreCase));
        }
        ...
        public override DownloadClientInfo GetStatus()
        {
            ...
            var configured = new[] { Settings.AudiobookCategory, Settings.EbookCategory }
                .Where(c => c.IsNotNullOrWhiteSpace())
                .Distinct(StringComparer.InvariantCultureIgnoreCase)
                .ToList();

            var matchedCategories = categories
                .Where(v => configured.Contains(v.Name, StringComparer.InvariantCultureIgnoreCase))
                .ToList();

            if (matchedCategories.Empty())
            {
                // Fall back to default category folder if we didn't find a configured category.
            ...
            {
                status.OutputRootFolders = matchedCategories
                    .Select(c => RemapRemoteToLocal(c.FullPath))
                    .Distinct()
                    .ToList();
            }
        ...
        private ValidationFailure TestConnectionAndVersion()
        {
            try
            {
                ...
                if (rawVersion.Equals("develop", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    {
                        IsWarning = true,
                        DetailedDescription = "Chaptarr may not be able to support new features added to SABnzbd when running develop versions."
                    };
                }
        ...
        private ValidationFailure TestGlobalConfig()
        {
            ...
            if (config.Misc.pre_check && !HasVersion(1, 1))
            {
                return new NzbDroneValidationFailure("", "Disable 'Check before download' option in Sabnbzd")
                {
                    ...
                    DetailedDescription = "Using Check before download affects Chaptarr ability to track new downloads. Also Sabnzbd recommends 'Abort jobs that cannot be completed' instead since it's more effective."
                };
            }
        ...
        private ValidationFailure TestCategory()
        {
            ...
            foreach (var pair in new[]
                     {
                         new { Field = nameof(SabnzbdSettings.AudiobookCategory), Value = Settings.AudiobookCategory },
                         new { Field = nameof(SabnzbdSettings.EbookCategory), Value = Settings.EbookCategory }
                     })
            {
                ...
                var category = categories.FirstOrDefault(v => v.Name == pair.Value);

                if (category == null)
                {
                    return new NzbDroneValidationFailure(pair.Field, "Category does not exist")
                    {
                ...
                {
                    return new NzbDroneValidationFailure(pair.Field, "Enable Job folders")
                    {
                        InfoLink = _proxy.GetBaseUrl(Settings, "config/categories/"),
                        DetailedDescription = "Chaptarr prefers each download to have a separate folder. With * appended to the Folder/Path Sabnzbd will not create these job folders. Go to Sabnzbd to fix it."
                    };
                }

                if (config.Misc.enable_tv_sorting && ContainsCategory(config.Misc.tv_categories, pair.Value))
                {
                    return new NzbDroneValidationFailure(pair.Field, "Disable TV Sorting")
                    {
                        InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                        DetailedDescription = "You must disable Sabnzbd TV Sorting for the category Chaptarr uses to prevent import issues. Go to Sabnzbd to fix it."
                    };
                }

                if (config.Misc.enable_movie_sorting && ContainsCategory(config.Misc.movie_categories, pair.Value))
                {
                    return new NzbDroneValidationFailure(pair.Field, "Disable Movie Sorting")
                    {
                        InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                        DetailedDescription = "You must disable Sabnzbd Movie Sorting for the category Chaptarr uses to prevent import issues. Go to Sabnzbd to fix it."
                    };
                }

                if (config.Misc.enable_date_sorting && ContainsCategory(config.Misc.date_categories, pair.Value))
                {
                    return new NzbDroneValidationFailure(pair.Field, "Disable Date Sorting")
                    {
                        InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                        DetailedDescription = "You must disable Sabnzbd Date Sorting for the category Chaptarr uses to prevent import issues. Go to Sabnzbd to fix it."
                    };
                }
            }
        ...
        public override object RequestAction(string action, IDictionary<string, string> query)
        {
            if (action == "getCategories")
            {
                try
                {
                    ...
                    // If API key is not set, return default option
                    if (string.IsNullOrWhiteSpace(Settings.ApiKey) && (string.IsNullOrWhiteSpace(Settings.Username) || string.IsNullOrWhiteSpace(Settings.Password)))
                    {
                        return new
                        {
                            options = new List<object>
                    ...
                    var config = _proxy.GetConfig(Settings);
                    var categories = config.Categories.OrderBy(c => c.Name).ToList();
                    ...
                    options.AddRange(categories.Select(cat => new
                    {
                        value = cat.Name,
                        name = cat.Name
                    }));
                ...
                {
                    _logger.Error(ex, "Unable to retrieve categories from SABnzbd");

                    // Return default options if we can't connect
                    return new
                    {
                        options = new List<object>
                        {
