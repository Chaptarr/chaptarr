    ...
    public class Pneumatic : DownloadClientBase<PneumaticSettings>
    {
        ...
        public override async Task<string> Download(RemoteBook remoteBook, IIndexer indexer)
        {
            ...
            else
            {
                ...
                request.RateLimitKey = remoteBook.Release.IndexerId > 0 ? remoteBook.Release.IndexerId.ToString() : null;
                var response = await indexer.ExecuteDownloadRequestAsync(request);
                using var stream = new MemoryStream(response.ResponseData ?? Array.Empty<byte>());
                _diskProvider.SaveStream(stream, nzbFile);
            }
