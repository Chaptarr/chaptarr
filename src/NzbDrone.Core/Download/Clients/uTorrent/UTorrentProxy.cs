        ...
        private void AuthenticateClient(HttpRequestBuilder requestBuilder, UTorrentSettings settings, bool reauthenticate = false)
        {
            ...
            if (cookies == null || authToken == null || reauthenticate)
            {
                ...
                try
                {
                    ...
                    // token.html is HTML. Avoid XML parsing to prevent XXE/DTD/expansion issues.
                    var match = Regex.Match(response.Content ?? string.Empty, "<div[^>]*id=[\"']token[\"'][^>]*>(?<token>[^<]*)</div>", RegexOptions.IgnoreCase);
                    authToken = match.Success ? WebUtility.HtmlDecode(match.Groups["token"].Value).Trim() : null;

                    if (authToken.IsNullOrWhiteSpace())
                    {
                        throw new DownloadClientException("Unable to extract uTorrent authentication token from token.html response");
                    }
                }
                catch (HttpException ex)
                {
