...
namespace NzbDrone.Core.Download
{
        ...
        where TSettings : IProviderConfig, new()
    {
        ...
        protected OsPath RemapRemoteToLocal(OsPath remotePath)
        {
            return _remotePathMappingService.RemapRemoteToLocal(Definition?.Id ?? 0, GetSettingsHost(), remotePath);
        }

        protected OsPath RemapLocalToRemote(OsPath localPath)
        {
            return _remotePathMappingService.RemapLocalToRemote(Definition?.Id ?? 0, GetSettingsHost(), localPath);
        }
        ...
        protected ValidationFailure TestFolder(string folder, string propertyName, bool mustBeWritable = true)
        {
            if (!_diskProvider.FolderExists(folder))
            {
                ...
                {
                    DetailedDescription = string.Format("The folder you specified does not exist or is inaccessible. Please verify the folder permissions for the user account '{0}', which is used to execute Chaptarr.", ProcessUserInfo.GetUserNameWithIds())
                };
            }
            ...
            if (mustBeWritable && !_diskProvider.FolderWritable(folder))
            {
                ...
                {
                    DetailedDescription = string.Format("The folder you specified is not writable. Please verify the folder permissions for the user account '{0}', which is used to execute Chaptarr.", ProcessUserInfo.GetUserNameWithIds())
                };
            }
