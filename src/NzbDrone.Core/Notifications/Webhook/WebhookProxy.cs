...
namespace NzbDrone.Core.Notifications.Webhook
{
    ...
    public class WebhookProxy : IWebhookProxy
    {
        ...
        public void SendWebhook(WebhookPayload body, WebhookSettings settings)
        {
            try
            {
                if (!Uri.TryCreate(settings.Url, UriKind.Absolute, out var webhookUri) ||
                    (webhookUri.Scheme != Uri.UriSchemeHttp && webhookUri.Scheme != Uri.UriSchemeHttps))
                {
                    throw new WebhookException("Webhook URL must be a valid absolute http(s) URL.");
                }

                if (CloudMetadataTargetPolicy.IsBlocked(webhookUri.Host))
                {
                    throw new WebhookException("Webhook URL is not allowed.");
                }
