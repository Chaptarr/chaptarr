...
namespace NzbDrone.Core.Notifications
{
    public interface INotificationFactory : IProviderFactory<INotification, NotificationDefinition>
    {
        ...
        List<INotification> OnBookAddedEnabled(bool filterBlockedNotifications = true);
    ...
    public class NotificationFactory : ProviderFactory<INotification, NotificationDefinition>, INotificationFactory
    {
        ...
        public List<INotification> OnReleaseImportEnabled(bool filterBlockedNotifications = true)
        {
            if (filterBlockedNotifications)
            {
                return FilterBlockedNotifications(GetAvailableProviders().Where(n => ShouldTriggerOnReleaseImport((NotificationDefinition)n.Definition))).ToList();
            }

            return GetAvailableProviders().Where(n => ShouldTriggerOnReleaseImport((NotificationDefinition)n.Definition)).ToList();
        }
        ...
        public List<INotification> OnRenameEnabled(bool filterBlockedNotifications = true)
        {
            if (filterBlockedNotifications)
            {
                return FilterBlockedNotifications(GetAvailableProviders().Where(n => ShouldTriggerOnRename((NotificationDefinition)n.Definition))).ToList();
            }

            return GetAvailableProviders().Where(n => ShouldTriggerOnRename((NotificationDefinition)n.Definition)).ToList();
        }
        ...
        public List<INotification> OnBookAddedEnabled(bool filterBlockedNotifications = true)
        {
            if (filterBlockedNotifications)
            {
                return FilterBlockedNotifications(GetAvailableProviders().Where(n => ((NotificationDefinition)n.Definition).OnBookAdded)).ToList();
            }

            return GetAvailableProviders().Where(n => ((NotificationDefinition)n.Definition).OnBookAdded).ToList();
        }
        ...
        public List<INotification> OnBookFileDeleteEnabled(bool filterBlockedNotifications = true)
        {
            if (filterBlockedNotifications)
            {
                return FilterBlockedNotifications(GetAvailableProviders().Where(n => ShouldTriggerOnBookFileDelete((NotificationDefinition)n.Definition))).ToList();
            }

            return GetAvailableProviders().Where(n => ShouldTriggerOnBookFileDelete((NotificationDefinition)n.Definition)).ToList();
        }
        ...
        {
            return definition.OnRename;
        }

        internal static bool ShouldTriggerOnBookFileDelete(NotificationDefinition definition)
        {
            return definition.OnBookFileDelete;
        }
        ...
        public override void SetProviderCharacteristics(INotification provider, NotificationDefinition definition)
        {
            ...
            definition.SupportsOnBookAdded = provider.SupportsOnBookAdded;
