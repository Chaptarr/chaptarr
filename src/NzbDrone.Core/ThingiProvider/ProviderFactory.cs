...
namespace NzbDrone.Core.ThingiProvider
{
        ...
        where TProvider : IProvider
    {
        ...
        {
            var provider = GetInstance(definition);
            ResolvePendingProviderSecrets(provider, false);
        ...
        {
            ResolvePendingProviderSecrets(definition, true);
        ...
        public virtual IEnumerable<TProviderDefinition> Update(IEnumerable<TProviderDefinition> definitions)
        {
            ...
            _providerRepository.UpdateMany(definitionsList);
        ...
        {
            return All().Where(c => c.Settings != null && c.Settings.Validate().IsValid).ToList();
        }
        ...
        private void ResolvePendingProviderSecrets(TProviderDefinition definition, bool consume)
        {
            ResolvePendingProviderSecrets(GetInstance(definition), consume);
        }

        private static void ResolvePendingProviderSecrets(TProvider provider, bool consume)
        {
            if (provider is IResolveProviderPendingSecrets resolver)
            {
                resolver.ResolveProviderPendingSecrets(consume);
            }
        }
