...
namespace NzbDrone.Core.Instrumentation
{
    public class ReconfigureLogging : IHandleAsync<ConfigFileSavedEvent>
    {
        ...
        public void Reconfigure()
        {
            ...
            // Matching diagnostics should be opt-in (Debug/Trace) and must not run at Info by default.
            SetMinimumLogLevel(rules, "appFileMatching", minimumLogLevel <= LogLevel.Debug ? minimumLogLevel : LogLevel.Off);
            SetLogRotation();

            //Database Logger
            SetMinimumLogLevel(rules, "DbLogger", minimumLogLevel <= LogLevel.Info ? LogLevel.Info : LogLevel.Off);
        ...
        private void SetSyslogParameters(string syslogServer, int syslogPort, LogLevel minimumLogLevel)
        {
            ...
            syslogTarget.Layout = new CleanseLayout
            {
                Inner = "[${level}] ${logger}: ${message} ${onexception:inner=${newline}${exception:format=ToString}}"
            };
