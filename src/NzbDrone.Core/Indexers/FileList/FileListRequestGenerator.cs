...
namespace NzbDrone.Core.Indexers.FileList
{
    public class FileListRequestGenerator : IIndexerRequestGenerator
    {
        ...
        {
            var pageableRequests = new IndexerPageableRequestChain();
            var searchCategories = Settings.Categories;
            ...
            pageableRequests.Add(GetRequest("search-torrents", searchCategories, string.Format("&type=name&query={0}+{1}", Uri.EscapeDataString(authorQuery), Uri.EscapeDataString(bookQuery))));
            pageableRequests.Add(GetRequest("search-torrents", searchCategories, string.Format("&type=name&query={0}+{1}", Uri.EscapeDataString(bookQuery), Uri.EscapeDataString(authorQuery))));
