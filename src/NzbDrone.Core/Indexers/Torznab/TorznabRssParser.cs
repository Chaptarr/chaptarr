    ...
    public class TorznabRssParser : TorrentRssParser
    {
        ...
        protected override ReleaseInfo ProcessItem(XElement item, ReleaseInfo releaseInfo)
        {
            ...
            if (torrentInfo != null)
            {
                ...
                var nfoValue = TryGetTorznabAttribute(item, "nfo", null);
                if (!nfoValue.IsNullOrWhiteSpace() && int.TryParse(nfoValue, out var nfoInt))
                {
                    torrentInfo.HasNfo = nfoInt == 1;
                }

                var infoValue = TryGetTorznabAttribute(item, "info", null);
                if (!infoValue.IsNullOrWhiteSpace())
                {
                    torrentInfo.NfoUrl = infoValue;
                }
            }
