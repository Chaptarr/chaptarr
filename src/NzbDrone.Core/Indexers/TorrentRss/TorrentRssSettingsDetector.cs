...
namespace NzbDrone.Core.Indexers.TorrentRss
{
    ...
    {
        TorrentRssIndexerParserSettings Detect(TorrentRssIndexerSettings settings, int? proxyId, int? indexerId);
    }
    ...
    public class TorrentRssSettingsDetector : ITorrentRssSettingsDetector
    {
        ...
        private readonly IIndexerHttpClientFactory _httpClientFactory;

        public TorrentRssSettingsDetector(IIndexerHttpClientFactory httpClientFactory, Logger logger)
        {
        ...
        /// <param name="proxyId">Proxy selected for the indexer</param>
        /// <param name="indexerId">Indexer identity used to isolate persisted cookies</param>
        /// <returns>Parsed Settings or <c>null</c></returns>
        public TorrentRssIndexerParserSettings Detect(TorrentRssIndexerSettings settings, int? proxyId, int? indexerId)
        {
            ...
            try
            {
                ...
                request.HttpRequest.RateLimitKey = indexerId?.ToString();

                HttpResponse httpResponse = null;
                try
                {
                    httpResponse = _httpClientFactory.GetClient(proxyId).Execute(request.HttpRequest);
                }
                catch (Exception ex)
                {
        ...
        private bool IsEZTVFeed(IndexerResponse response)
        {
            ...
            // Detect EZTV feeds that use a DTD without enabling DTD parsing (prevents entity-expansion DoS).
            if (content.IndexOf("http://xmlns.ezrss.it/0.1/dtd/", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                _logger.Trace("Identified feed as EZTV compatible by EZTV DTD");
                return true;
            }

            using (var xmlTextReader = XmlReader.Create(new StringReader(content), new XmlReaderSettings { DtdProcessing = DtdProcessing.Ignore, ValidationType = ValidationType.None, IgnoreComments = true, XmlResolver = null }))
            {
