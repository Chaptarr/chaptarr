...
namespace NzbDrone.Core.Indexers
{
        ...
        where TSettings : IIndexerSettings, new()
    {
        ...
        protected readonly IIndexerHttpClientFactory _httpClientFactory;
        protected IIndexerHttpClient _httpClient => _httpClientFactory.GetClient(((IndexerDefinition)Definition)?.ProxyId);
        ...
        public HttpIndexerBase(IIndexerHttpClientFactory httpClientFactory, IIndexerStatusService indexerStatusService, IConfigService configService, IParsingService parsingService, Logger logger)
        ...
        protected virtual async Task<IList<ReleaseInfo>> FetchReleases(Func<IIndexerRequestGenerator, IndexerPageableRequestChain> pageableRequestChainSelector, bool isRecent = false)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < pageableRequestChain.Tiers; i++)
                {
                    ...
                    var releasesBeforeTier = releases.Count;
                    ...
                    // A later tier only runs when every earlier one came back empty, so when a
                    // search finds nothing this says which attempts were actually made, and when
                    // it finds something it says which attempt found it.
                    if (pageableRequestChain.Tiers > 1)
                    {
                        var tierReleaseCount = releases.Count - releasesBeforeTier;

                        _logger.Debug("Search tier {0}/{1} returned {2} release(s){3}",
                            i + 1,
                            pageableRequestChain.Tiers,
                            tierReleaseCount,
                            tierReleaseCount > 0 ? ", skipping the remaining tiers" :
                                i + 1 < pageableRequestChain.Tiers ? ", trying the next tier" : string.Empty);
                    }
        ...
        protected virtual async Task<IndexerResponse> FetchIndexerResponse(IndexerRequest request)
        {
            ...
            // Persist cookies from responses and any cookies we set on requests
            request.HttpRequest.StoreResponseCookie = true;
            request.HttpRequest.StoreRequestCookie = true;
        ...
        {
            // Use the indexer's HTTP client which is configured with proxy settings
            // Ensure cookie jar is applied and updates are persisted
            request.StoreRequestCookie = true;
            request.StoreResponseCookie = true;

            try
            {
                _ = request.Headers.ContainsKey("Cookie");
            }
            catch { }
