...
namespace NzbDrone.Core.Indexers.Newznab
{
    public class NewznabRssParser : RssParser
    {
        ...
        protected override ReleaseInfo ProcessItem(XElement item, ReleaseInfo releaseInfo)
        {
            ...
            // Indexer-provided extended metadata hints (used by narrator metadata enrichment)
            var nfoValue = TryGetNewznabAttribute(item, "nfo", null);
            if (!nfoValue.IsNullOrWhiteSpace() && int.TryParse(nfoValue, out var nfoInt))
            {
                releaseInfo.HasNfo = nfoInt == 1;
            }

            var infoValue = TryGetNewznabAttribute(item, "info", null);
            if (!infoValue.IsNullOrWhiteSpace())
            {
                releaseInfo.NfoUrl = infoValue;
            }
        ...
        protected string TryGetNewznabAttribute(XElement item, string key, string defaultValue = "")
        {
            ...
            if (attrElement != null)
            {
                ...
                if (attrValue != null)
                {
                    return DecodeAttributeValue(attrValue.Value);
                }
            }
        ...
        protected List<string> TryGetMultipleNewznabAttributes(XElement item, string key)
        {
            ...
            foreach (var element in attrElements)
            {
                ...
                if (attrValue != null)
                {
                    results.Add(DecodeAttributeValue(attrValue.Value));
                }
            }
