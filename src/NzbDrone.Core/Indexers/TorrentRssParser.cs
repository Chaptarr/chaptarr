...
namespace NzbDrone.Core.Indexers
{
    public class TorrentRssParser : RssParser
    {
        ...
        protected override ReleaseInfo ProcessItem(XElement item, ReleaseInfo releaseInfo)
        {
            ...
            result.InfoHash = TorrentHashResolver.NormalizeKnownHash(GetInfoHash(item));

            if (result.InfoHash.IsNullOrWhiteSpace() && result.MagnetUrl.IsNotNullOrWhiteSpace())
            {
                try
                {
                    result.InfoHash = TorrentHashResolver.GetSupportedHashOrNull(MagnetLink.Parse(result.MagnetUrl).InfoHashes);
                }
                catch
                {
                }
        ...
        protected virtual string GetInfoHash(XElement item)
        {
            ...
            if (magnetUrl.IsNotNullOrWhiteSpace())
            {
                try
                {
                    return TorrentHashResolver.GetSupportedHashOrNull(MagnetLink.Parse(magnetUrl).InfoHashes);
                }
                catch
                {
                }
