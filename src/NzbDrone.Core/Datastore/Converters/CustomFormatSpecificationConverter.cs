...
namespace NzbDrone.Core.Datastore.Converters
{
    ...
    {
        private static readonly IReadOnlyDictionary<string, Type> AllowedSpecificationTypes = BuildAllowedSpecificationTypes();
        ...
        public override List<ICustomFormatSpecification> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            while (reader.TokenType == JsonTokenType.StartObject)
            {
                ...
                if (string.IsNullOrWhiteSpace(typename) || !AllowedSpecificationTypes.TryGetValue(typename, out var type))
                {
                    throw new JsonException($"Unknown custom format specification type '{typename}'");
                }
        ...
        private static IReadOnlyDictionary<string, Type> BuildAllowedSpecificationTypes()
        {
