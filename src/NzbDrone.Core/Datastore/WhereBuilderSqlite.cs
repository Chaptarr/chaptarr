...
namespace NzbDrone.Core.Datastore
{
    public class WhereBuilderSqlite : WhereBuilder
    {
        ...
        private void ParseEnumerableContains(MethodCallExpression body)
        {
            ...
            {
                // Static method. Most commonly Enumerable.Contains(source, item) on older TFMs,
                // but on net10+ expression trees may emit MemoryExtensions.Contains(ReadOnlySpan<T>, T)
                // for array contains (via implicit conversion to ReadOnlySpan<T>).
                if (body.Arguments.Count != 2)
                {
                    throw new NotSupportedException("Unexpected form of Enumerable.Contains");
                }

                if (body.Method.DeclaringType == typeof(Enumerable) || body.Method.DeclaringType == typeof(Queryable))
                {
                ...
                {
                    list = UnwrapSpan(body.Arguments[0]);
                    item = body.Arguments[1];
                }
                else
                {
        ...
        {
            // net10+ expression trees represent `array.Contains(x)` as MemoryExtensions.Contains(op_Implicit(array), x),
            // where op_Implicit converts T[] -> ReadOnlySpan<T>. Peel that implicit conversion so Visit() can parameterize the array.
            if (expression is MethodCallExpression methodCall &&
                methodCall.Method.Name == "op_Implicit" &&
                methodCall.Arguments.Count == 1 &&
                expression.Type.IsGenericType)
            {
                var genericDefinition = expression.Type.GetGenericTypeDefinition();
                if (genericDefinition == typeof(ReadOnlySpan<>) || genericDefinition == typeof(Span<>))
                {
