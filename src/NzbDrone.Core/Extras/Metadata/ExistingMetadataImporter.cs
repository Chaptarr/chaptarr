        ...
        public override IEnumerable<ExtraFile> ProcessFiles(Author author, List<string> filesOnDisk, List<string> importedFiles)
        {
            ...
            foreach (var possibleMetadataFile in filterResult.FilesOnDisk)
            {
                foreach (var consumer in _consumers)
                {
                    ...
                    // No tag-based augmentation during extras import; BookId was already resolved via parsing

                    metadata.Extension = Path.GetExtension(possibleMetadataFile);
                    if (!ExtraFilePathHelper.TryGetRelativePath(author, possibleMetadataFile, out var relativePath, out _))
                    {
                        continue;
                    }
