                                ...
                                IHandle<AuthorRenamedEvent>
    {
        ...
        public void ImportExtraFiles(LocalBook localBook, BookFile bookFile, bool isReadOnly)
        {
            ...
            foreach (var matchingFilename in filteredFilenames)
            {
                ...
                if (string.IsNullOrWhiteSpace(extension) || !wantedExtensions.Contains(extension))
                {
                    continue;
                }
        ...
        public void Handle(MediaCoversUpdatedEvent message)
        {
            if (message.Book != null)
            {
                ...
                var bookFiles = _mediaFileService.GetFilesByBook(message.Book.Id);
            ...
            var refreshedAuthor = message.Author;
            if (refreshedAuthor == null) return;

            var refreshedBookFiles = GetBookFiles(refreshedAuthor.Id);

            foreach (var extraFileManager in _extraFileManagers)
            {
                extraFileManager.CreateAfterAuthorScan(refreshedAuthor, refreshedBookFiles);
            }
        }
        ...
        public void Handle(TrackFolderCreatedEvent message)
        {
            ...
            foreach (var extraFileManager in _extraFileManagers)
            {
                extraFileManager.CreateAfterBookImport(author, edition.Book, message.AuthorFolder, message.BookFolder);
            }
        }
        ...
        {
            var author = message.Author;
            var bookFiles = message.RenamedFiles?
                .Select(r => r.BookFile)
                .Where(bf => bf != null)
                .DistinctBy(bf => bf.Id)
                .ToList()
