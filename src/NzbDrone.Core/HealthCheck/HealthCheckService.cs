...
namespace NzbDrone.Core.HealthCheck
{
                                      ...
                                      IHandleAsync<IEvent>
    {
        ...
        public void Execute(CheckHealthCommand message)
        {
            ...
            {
                // A user-invoked re-check is an explicit retry request: clear the metadata
                // server circuit so its next request re-tests availability instead of
                // sitting out the remaining cooldown. Scheduled runs keep the ladder intact.
                _metadataServerHealthGate.Reset();
