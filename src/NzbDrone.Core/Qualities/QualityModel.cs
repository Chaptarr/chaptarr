...
namespace NzbDrone.Core.Qualities
{
    public class QualityModel : IEmbeddedDocument, IEquatable<QualityModel>, IComparable
    {
        ...
        // Multi-format support for releases with multiple detected qualities
        [JsonIgnore]
        public List<Quality> DetectedQualities { get; set; } = new List<Quality>();

        // Helper properties for GUI display
        [JsonIgnore]
        public bool IsMultiFormat => DetectedQualities?.Count > 1;

        [JsonIgnore]
        public string AllDetectedFormats => DetectedQualities != null && DetectedQualities.Any() ?
            string.Join(", ", DetectedQualities.Select(q => q.Name)) : "";
