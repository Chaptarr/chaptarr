...
namespace NzbDrone.Core.Update
{
    public class InstallUpdateService : IExecute<ApplicationUpdateCommand>, IExecute<ApplicationUpdateCheckCommand>, IHandle<ApplicationStartingEvent>
    {
        ...
        private bool InstallUpdate(UpdatePackage updatePackage)
        {
            ...
            {
                _logger.Warn("Temporary location '{0}' has less than 1 GB free space, Chaptarr may not be able to update itself.", tempFolder);
            }

            var safePackageFileName = SanitizeUpdatePackageFileName(updatePackage.FileName);
            var packageDestination = Path.Combine(updateSandboxFolder, safePackageFileName);
            ...
            _logger.ProgressInfo("Chaptarr will restart shortly.");
        ...
        private void InstallUpdateWithScript(string updateSandboxFolder)
        {
            ...
            _logger.Info("Removing Chaptarr.Update");
        ...
        private static string SanitizeUpdatePackageFileName(string fileName)
        {
            if (fileName.IsNullOrWhiteSpace())
            {
                throw new UpdateFailedException("Update package filename is missing");
            }

            // Never allow writing outside the update sandbox via crafted filenames.
            // Drop any directory components and normalize invalid characters.
            var candidate = Path.GetFileName(fileName).Trim();
            if (candidate.IsNullOrWhiteSpace() || candidate == "." || candidate == "..")
            {
                throw new UpdateFailedException("Update package filename is invalid");
            }
        ...
        private void EnsureAppDataSafety()
        {
            ...
            {
                throw new UpdateFailedException("Your Chaptarr configuration '{0}' is being stored in application folder '{1}' which will cause data lost during the upgrade. Please remove any symlinks or redirects before trying again.", _appFolderInfo.AppDataFolder, _appFolderInfo.StartUpFolder);
            }
        }
        ...
        public void Execute(ApplicationUpdateCommand message)
        {
            ...
            if (latestAvailable != null)
            {
                try
                {
                    ...
                    _logger.ProgressDebug("Restarting Chaptarr to apply updates");
                }
