...
import queueRowStyles from '../../Activity/Queue/QueueRow.css';
  ...
  paged: {
    ...
    columns: [
      ...
      },
      {
        ...
        isSortable: true,
        isVisible: true,
        className: queueRowStyles.quality
      },
      {
        name: 'convertToQuality',
        label: () => translate('WillConvertTo'),
        isSortable: true,
        isVisible: true,
        className: queueRowStyles.convertToQuality
      },
      {
        name: 'customFormats',
        label: 'Formats',
      ...
      },
      {
        ...
        isSortable: true,
        isVisible: false,
        className: queueRowStyles.releaseTitle
      },
      {
        name: 'size',
...
export const CANCEL_QUEUE_CONVERSION = 'queue/cancelQueueConversion';
...
export const cancelQueueConversion = createThunk(CANCEL_QUEUE_CONVERSION);
...
export const actionHandlers = handleThunks({
  ...
  [CANCEL_QUEUE_CONVERSION]: function(getState, payload, dispatch) {
    const id = payload.id;

    dispatch(updateItem({ section: paged, id, isCancellingConversion: true }));

    const promise = createAjaxRequest({
      url: `/queue/cancelconversion/${id}`,
      method: 'POST'
    }).request;
