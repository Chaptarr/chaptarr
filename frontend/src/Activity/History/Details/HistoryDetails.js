...
function formatConversionTagMode(tagMode) {
  if (tagMode === 'clean') {
    return 'Clean matched tags';
  }

  if (tagMode === 'preserve') {
    return 'Preserve source tags';
  }
...
function HistoryDetails(props) {
  ...
  if (eventType === 'bookFileConverted' || eventType === 'bookFileConversionFailed') {
    const {
      sourcePath,
      sourceFileCount,
      sourceQuality,
    ...
    return (
      <DescriptionList>
...

        {
          !!tagMode &&
            <DescriptionListItem
              title={translate('ConversionTags')}
              data={formatConversionTagMode(tagMode)}
            />
        }
