...
import AuthorDetailsControls from './AuthorDetailsControls';
import SelectAuthorRootFolderModal from './SelectAuthorRootFolderModal';
...
class AuthorDetails extends Component {
  ...
  onSelectRootFolderPress = (mediaType) => {
    this.setState({
      isSelectRootFolderModalOpen: true,
      selectRootFolderMediaType: mediaType === 'ebook' ? 'ebook' : 'audiobook'
    });
  };
  ...
  onSelectRootFolderModalClose = () => {
    this.setState({ isSelectRootFolderModalOpen: false });
  };
  ...
  render() {
    ...
    const manualImportFolder = selectedMediaType === 'ebook' ? ebookFolder : audiobookFolder;
      ...
      <PageContent title={authorName}>
        ...
        <PageContentBody innerClassName={styles.innerContentBody}>
          <SwipeHeaderConnector
            ...
            nextComponent={(width) => <AuthorDetailsHeaderConnector authorId={nextAuthor.id} width={width} selectedMediaType={selectedMediaType} />}
            prevLink={`/author/${previousAuthor.id}`}
            prevComponent={(width) => <AuthorDetailsHeaderConnector authorId={previousAuthor.id} width={width} selectedMediaType={selectedMediaType} />}
            currentComponent={(width) => <AuthorDetailsHeaderConnector authorId={id} width={width} selectedMediaType={selectedMediaType} />}
          >
          ...
          <div className={styles.contentContainer}>
            {
              isPopulated &&
                <AuthorDetailsControls
                  authorId={id}
                  selectedMediaType={selectedMediaType || 'audiobook'}
                  onMediaTypeChange={onMediaTypeChange}
                  onSelectRootFolderPress={this.onSelectRootFolderPress}
                  hasAudiobookRootFolder={hasAudiobookRootFolder}
                  hasEbookRootFolder={hasEbookRootFolder}
                  hasAudiobookRootFoldersAvailable={hasAudiobookRootFoldersAvailable}
                  hasEbookRootFoldersAvailable={hasEbookRootFoldersAvailable}
                  authorHasAudiobookConfig={authorHasAudiobookConfig}
                  ...
                  <TabPanel>
                    ...
                    />
	                  ...
	                  <TabPanel>
	                    {
	                      (() => {
	                        const rootFolderStatus = getAuthorMediaTypeRootFolderStatus(this.props, selectedMediaType);
	                        if (!rootFolderStatus.hasRootFolder) {
	                          return (
	                            <Alert kind={kinds.WARNING}>
	                              {translate('AuthorDetailsNoMediaTypeRootFolder', { mediaType: selectedMediaType })}
	                            </Alert>
	                          );
	                        }

	                        if (!(isPopulated && hasSeries)) {
	                          return null;
	                        }
          ...
          />
	          ...
	          <EditAuthorModalConnector
	            ...
	            authorId={id}
	            initialMediaType={selectedMediaType || 'audiobook'}
	            onModalClose={this.onEditAuthorModalClose}
	            onDeleteAuthorPress={this.onDeleteAuthorPress}
	          />

	          <SelectAuthorRootFolderModal
	            isOpen={isSelectRootFolderModalOpen}
	            authorId={id}
	            mediaType={selectRootFolderMediaType || selectedMediaType || 'audiobook'}
	            onModalClose={this.onSelectRootFolderModalClose}
	          />
...
AuthorDetails.propTypes = {
  ...
  visibleSeriesCount: PropTypes.number,
