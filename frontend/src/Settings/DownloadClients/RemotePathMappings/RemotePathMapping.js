...
class RemotePathMapping extends Component {
  ...
  render() {
    ...
    const scope = downloadClientId > 0 ? `${downloadClientName || `Download client ${downloadClientId}`} (${host})` : host;
    ...
    return (
      ...
      >
        <div className={styles.host}>{scope}</div>
...
RemotePathMapping.propTypes = {
  ...
  downloadClientId: PropTypes.number,
