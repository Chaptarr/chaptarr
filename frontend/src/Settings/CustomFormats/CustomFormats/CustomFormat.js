...
import getNarratorNames from './Specifications/getNarratorNames';
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <MediaTypeScope mediaType={appliesTo ?? 'both'} />
        ...
        <div className={styles.formats}>
          {
            specifications.map((item, index) => {
              ...
              if (narratorNames.length) {
                return narratorNames.map((narratorName, narratorIndex) => {
                  return (
                    <Label
                      className={styles.label}
                      key={`${index}-${narratorIndex}`}
                      kind={item.negate ? kinds.DANGER : kinds.SUCCESS}
                    >
                      {narratorName}
                    </Label>
                  );
...
CustomFormat.propTypes = {
  ...
  appliesTo: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
