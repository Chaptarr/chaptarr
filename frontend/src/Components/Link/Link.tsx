...
function Link(props: LinkProps) {
  ...
  if (to) {
    const isUnsafeProtocol = /^\s*(javascript|data):/i.test(to);
    ...
    if (isUnsafeProtocol) {
      ...
      // @ts-ignore
      el = RouterLink;
      linkProps.to = `${window.Chaptarr.urlBase}/${to.replace(/^\//, '')}`;
  ...
  // Only attach onClick handler if we have an onPress or if it's not an external link
  if (onPress || !to || !/^https?:\/\//i.test(to)) {
