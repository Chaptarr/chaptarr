...
function valueIncludes(value, key) {
  return Array.isArray(value) && value.some((selectedValue) => `${selectedValue}` === `${key}`);
}

function getFetchErrorText(error) {
  if (!error) {
    return null;
  }
  ...
  if (responseJSON && typeof responseJSON === 'object') {
    return responseJSON.message || responseJSON.error || error.responseText || null;
  }

  return error.responseText || (error.status && error.statusText ? `${error.status} ${error.statusText}` : null);
}
...
function getSelectedIndex(props) {
  ...
  if (Array.isArray(value)) {
    return values.findIndex((v) => {
      return value.length && valueIncludes(value, v.key);
    });
  }
...
function isSelectedItem(index, props) {
  ...
  if (Array.isArray(value)) {
    return valueIncludes(value, values[index].key);
  }
...
class EnhancedSelectInput extends Component {
  ...
  onSelect = (value) => {
    if (Array.isArray(this.props.value)) {
      let newValue = null;
      const index = this.props.value.findIndex((selectedValue) => `${selectedValue}` === `${value}`);
      if (index === -1) {
        newValue = this.props.values
          .map((v) => v.key)
          .filter((v) => (`${v}` === `${value}`) || valueIncludes(this.props.value, v));
      } else {
  ...
  render() {
    ...
    const selectedOptionProps = selectedOption ? { ...selectedOption } : {};
    ...
    const selectedOptionValue = selectedOptionProps.value;
    delete selectedOptionProps.value;

    const displayValue = isMultiSelect ?
      selectedValue :
      (selectedOptionValue ?? selectedValue);

    const fetchErrorText = isFetching ? null : getFetchErrorText(fetchError);
                        ...
                        >
                          {
                            values.map((v, index) => {
                              ...
                              const parentSelected = hasParent && valueIncludes(value, v.parentKey);
                              return (
                                <OptionComponent
                                  key={v.key}
                                  id={v.key}
