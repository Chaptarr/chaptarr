...
function ProviderFieldFormGroup(props) {
  ...
  // Determine the kind for tag fields - make language-related tags green
  let kind = otherProps.kind;
  const isNarratorNamesField =
    otherProps.providerData?.implementation === 'NarratorNamesSpecification' &&
    name === 'names';
  if (!kind && type === 'tag') {
    // Check if this is a language-related field
    if (name === 'searchLanguages' ||
        name === 'allowedLanguages' ||
        label?.toLowerCase().includes('language') ||
        isNarratorNamesField) {
      kind = kinds.SUCCESS; // Green for language tags
    }
  }
