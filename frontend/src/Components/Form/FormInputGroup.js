...
import ProxySelectInputConnector from './ProxySelectInputConnector';
...
function getComponent(type) {
  switch (type) {
    ...
    case inputTypes.PROXY_SELECT:
      return ProxySelectInputConnector;
...
function FormInputGroup(props) {
  ...
  const finalButtonsArray = buttonsArray;

  const lastButtonIndex = finalButtonsArray.length - 1;
  const hasButton = !!finalButtonsArray.length;
  ...
  return (
    <div className={containerClassName}>
      <div className={className}>
        ...
        {
          finalButtonsArray.map((button, index) => {
